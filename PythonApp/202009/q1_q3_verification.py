...
model = Sequential()
model.add(Dense(input_dim=2, units=1))  #入力の次元数はいくつで、出力の次元数はいくつか
model.add(Activation('sigmoid'))        #活性化関数は何か
model.compile(loss='binary_crossentropy', optimizer=SGD(lr=0.1)) #どの損失関数(=クロスエントロピー)をつかって、どのように方法(確率勾配降下法)で学習させるか

# 学習の実行
#model.ﬁt(X, Y, batch_size=1, epochs=200)
model.ﬁt(X, Y, batch_size=1, epochs=20)
