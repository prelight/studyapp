...
def glob_images(path, label, max_photo):
  ...
  for i, f in enumerate(files):
    ...
    data = data.reshape(photo_size, photo_size, 3)#これは必要？
    X.append(data)
    y.append(label)
...
def make_dataset(max_photo, outfile):
  ...
  glob_images("201712_week2/sakura-ok", 0, max_photo)    # 桜
  glob_images("201712_week2/sunflower-ok", 1, max_photo) # ヒマワリ
  glob_images("201712_week2/rose-ok", 2, max_photo)      # バラ
...
# データセットを2種類作成する
make_dataset(100, "201712_week2/photo-min.npz")
make_dataset(300, "201712_week2/photo.npz")
